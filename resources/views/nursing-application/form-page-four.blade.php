...
<div class="w-full">
        ...
        <div class="bg-white shadow-sm mt-0 lg:mt-5">
            <div class="max-w-7xl mx-auto px-4 py-2 flex justify-between items-center">
                <div class="flex items-center gap-2 text-sm">
                    <span class="text-blue-600 font-semibold">Step 4</span>
    ...
    <!-- Main Content -->
    <form method="POST" action="{{ route('form.step3') }}">
    @csrf
    <div class="w-full pt-32  md:pt-40 overflow-y-autob h-[900px] lg:h-[800px]">
        <div class="flex flex-col items-center justify-center">
            <!-- Heading Text -->
            <div class="px-6 text-center">
                <h1 class="font-sans text-xl font-semibold md:text-4xl text-slate-600">
                    In which of these cities would you like to work in?<span class="text-red-600">*</span>
                </h1>
            </div>

            <div class="block w-full px-6 mt-10 md:justify-center md:flex md:mt-20">
                @php
                    $workCountries = session('form.step2.work_country', []);  // Retrieve countries from session
                @endphp

                @foreach($workCountries as $index => $country)
                <div class="grid grid-cols-1 md:gap-4 lg:gap-1 lg:w-4/12">
                    <div class="flex flex-col mb-4">
                        ...
                        <div class="flex flex-col w-full px-8 mb-4 md:space-y-2">
                            ...
                            <select id="states-{{ $index }}" name="states[{{ $index }}][]" class="block w-full px-6 py-3 text-gray-500 border-2 border-gray-400 focus:outline-none" multiple="multiple"></select>
                        </div>
                    </div>
                    <div class="flex flex-col w-full px-8 mb-4 md:space-y-2">
                        <label for="postal-codes-{{ $index }}" class="block font-sans text-base font-semibold text-black md:text-lg">Postal Codes</label>
                        <input type="text" id="postal-codes-{{ $index }}" name="postal_codes[{{ $index }}]" class="block w-full px-6 py-3 text-gray-500 border-2 border-gray-400 focus:outline-none" readonly>
                    </div>
                </div>
                @endforeach
            </div>
            @if($errors->any())
                <div class="mt-6 font-sans text-base text-red-600">
                    <ul>
        ...
        <div class="md:absolute w-full px-6 mt-12 md:bottom-24">
            <div class="flex justify-between">
                <!-- Previous button -->
                <a href="{{ route('form.step2') }}" class="w-32 md:w-44 px-2 py-2 md:px-4 md:py-4 border-none bg-[#0693e3] text-base md:text-xl font-medium transition duration-300 delay-100 hover:bg-[#2ea3f2] text-white flex items-center justify-center">
                ...
                <!-- Next button -->
                <button type="submit" class="w-24 md:w-44 px-2 py-2 md:px-4 md:py-4 border-none bg-[#0693e3] text-base md:text-xl font-medium transition duration-300 delay-100 hover:bg-[#2ea3f2] text-white flex items-center justify-center">
                    <span>Next</span>
